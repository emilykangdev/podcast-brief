...
- **`.mjs` vs `.js` — two module worlds, never cross them:**
  ...
  - `.mjs` files (worker/script code): Real ESM, run with plain `node server.mjs` on Railway (Node 20). No transpiler.
  - **NEVER import a `.js` file from a `.mjs` file.** Plain Node treats `.js` as CommonJS (no `"type": "module"` in package.json), regardless of Node's major version, so ESM syntax in `.js` files causes a crash under plain Node. If worker code needs something from a `.js` file, either duplicate the value inline or create a shared `.mjs` module.
